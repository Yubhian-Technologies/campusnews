...
/**
 * College + department manager scoped to a single (fixed) location — used by a
 * Location News Head, who can manage colleges/departments in their own location
 * but not locations themselves. Backed by the same /api/admin/org endpoints,
 * which enforce the scope server-side.
 */
import { useCallback, useState } from "react";
import { toast } from "sonner";
import { ChevronRight, Plus, Trash2, Loader2 } from "lucide-react";
import type { College, Department } from "@/lib/org/types";
...
export function ScopedOrgManager({
  locationId,
  locationName,
  initialColleges,
}: {
  ...
  const loadDepartments = useCallback(async (collegeId: string) => {
    try {
      setDepartments(await listOrgAdmin<Department>("departments", { collegeId }));
    } catch (err) {
  ...
  return (
    <div className="grid gap-4 sm:grid-cols-2">
      <Column
        title={`Colleges in ${locationName}`}
...
function Column({
  ...
  async function add() {
    ...
    try {
      ...
      toast.success(`${title.split(" ")[0].replace(/s$/, "")} added.`);
      setName("");
      setId("");
      setExtraVal("");
      setIdEdited(false);
      ...
      {disabled ? (
        ...
        <>
          ...
          <div className="mt-4 space-y-2 border-t pt-3">
            ...
            <Button size="sm" className="w-full" disabled={busy} onClick={add}>
              {busy ? (
